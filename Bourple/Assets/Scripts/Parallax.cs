...
public class Parallax : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < backgrounds.Length; i++)
        {
            ...
            float dupeX = backgrounds[i].GetComponent<SpriteRenderer>().bounds.size.x;
            for (int j = 1; j < duplicateAmount; j++)
            {
                GameObject bg = Instantiate(backgrounds[i].gameObject, new Vector3(dupeX * j, 0, backgrounds[i].position.z), Quaternion.identity);
                bg.transform.SetParent(backgrounds[i].transform.parent);
            }
    ...
    void Update()
    {
        ...
        previousCamPos = cam.position;
        transform.position = new Vector3(0, cam.position.y);
